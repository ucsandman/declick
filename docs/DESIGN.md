...
There is one more section on /, id agents, between governance and commands. The h2 is "Any agent with a shell, not only Claude Code", and it makes two claims: `declick add` writes the adapter's SKILL.md into every agent skills directory that already exists on the machine, naming four agents by their paths, with DECLICK_SKILLS as a comma-separated override for anything else; and a custom agent on the Anthropic SDK, the OpenAI SDK or Managed Agents needs exactly one tool, which runs `declick run <adapter> <verb> --json` and hands stdout back as the tool result. The code writes a fifth path, `~/.agents/skills`, which the section does not name; the next recapture adds it. The block beside that prose is the only code on the page that is source rather than captured output, and its caption says so in the first three words. It is an eight line Node function, and it was run on Node 24 against the package published to npm, where it returned a real row for `get-user-by-name user1` and a real exit 2 envelope for an unknown verb. It was re-run for 0.3.1 against an mcp adapter's `list-notes` with the same result. Being source, it is outside the recapture procedure; it gets rechecked when the run interface or the package entry point changes, not at every release. The governance no-key figure is the opposite case. It is captured output, and what it proves is what the tool writes to stderr when no DASHCLAW_API_KEY is set, so it has to be recaptured whenever the guard's stderr behaviour changes. 0.3.1 is exactly that change: the warning line is gone and the envelope carries `governance.enabled` false with the reason `no guard configured`, so that block and the sentence above it are the first thing to recapture.
...
The install section's second figure shows `petstore get-user-by-name user1 --fields username,email` (a real call, exit 0, petstore's user endpoints declare no auth) followed by `petstore get-pet-by-id 7 --json false` (exit 4, the spec declares an API key). Recapture both with the same env prefix, and replace the vault path in the exit 4 line with `~/.creds/vault.env`. docs/demo.gif is the README demo: a headless Playwright capture (1000x880, 2x replay speed, cropped to the terminal card, 8 fps, 800 px wide, 48 colours) of the declick Replay artifact at https://claude.ai/code/artifact/5b5fee66-1b5f-4d83-9d44-9f67303bb726, whose beats are the verbatim 0.3.2 session. Re-record it whenever the replay changes.

## The store section

One more section on /, id store, between setup and desktop, added for 0.7.0 with an anchor in the hero section list. The h2 is "Push an adapter once. Every machine on the team pulls it." It makes three claims: a store is a directory the team shares, a git checkout or a read-only https base of export bundles plus an index; push writes the bundle and the index entry and pull installs what changed; bundles carry auth key names and never values. The figure is captured output of `declick store set`, `declick store push petstore` and, from a second home with DECLICK_STORE pointing at the same folder, `declick store pull`, all with `--json false`. Recapture with the fixture copied to a stand-in home first (`cp fixtures/petstore.json <scratch>/home/petstore.json`, then `add` from that path), because the pulled row prints the adapter's source and the ~-shortening rule covers a home path, not the repo's fixtures path. The store path in the capture is shortened to `~/team/declick-store` the same way. The git line reads `git: none` on purpose: the captured store is a plain folder, and a git checkout would print `pushed` and `pulled`, which the README shows in prose rather than the page.

## Analytics and Search Console

The page ships one script: Vercel Web Analytics, loaded deferred from /_vercel/insights/script.js on the same origin (no cookies, no third-party host). It is the only JavaScript on the site and nothing on the page depends on it; with JS disabled the page is unchanged. Enabled in the Vercel project on 2026-09-03. Google Search Console verifies https://declick.dev/ through the google-site-verification meta tag in the head of index.html; removing that tag revokes the verification. Bing Webmaster Tools verifies through the msvalidate.01 meta tag next to it (the code is account-wide and public by design); the launch CLI submitted the sitemap and home URL to Bing on 2026-09-03 and records that in the gitignored .launch/ledger.json.

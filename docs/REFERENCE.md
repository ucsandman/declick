...
Two pushes at the same instant do not lose each other: the index rewrite runs under `<store>/index.lock`, a directory taken with an atomic mkdir (which works on local disks, SMB and NFS shares alike), so each push re-reads the index inside the lock and keeps what the other one added. A push waits up to 5 seconds for the lock, then exits 1 naming it; a lock older than 30 seconds belongs to a push that died and is taken over. On a directory store the bundles on disk are the truth: a listing or pull merges any `<name>.json` the index does not mention, so an index that lags (a push killed between its two writes, a bundle dropped in by hand) hides nothing.

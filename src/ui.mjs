...
function page(rows) {
  const tr = rows.map(rowHtml).join('\n');
...
<form id="add"><input name="source" placeholder="source: spec.json | https://... | app:window | mcp:server" size="40" required> <input name="name" placeholder="name (optional)"> <input name="goal" placeholder="goal (optional)"> <button>add</button></form>
...
document.addEventListener('click', async e => {
  ...
  const r = await fetch('/api/' + encodeURIComponent(name) + '/' + action, { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{}' }).then(r => r.json());
  if (show(r)) location.reload(); else { b.disabled = false; b.textContent = action; }
});
document.getElementById('add').addEventListener('submit', async e => {
  ...
  const r = await fetch('/api/add', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(body) }).then(r => r.json());
  ...
  const server = createServer((req, res) => {
    ...
    (async () => {
      ...
      if (req.method === 'POST' && url.pathname === '/api/add') {
        ...
        for (const [flag, key] of [['--name', 'name'], ['--goal', 'goal'], ['--verb', 'verb'], ['--recipes', 'recipes']]) if (body[key]) args.push(flag, body[key]);
